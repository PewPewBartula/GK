...
float fi2 = 0.0;
static int zoom = 30;
GLfloat theta1 = 0.0;   // kąt obrotu zrodla
GLfloat theta2 = 0.0;   // kat obrotu zrodla
...
void RenderScene(void)
{
    ...
    if (status == 1) {
        theta1 += delta_x * pix2anglex;    // modyfikacja kąta obrotu o kat proporcjonalny
        fi += delta_y * pix2angley;
    }

    if (status == 2) {
        theta2 += delta_x * pix2anglex;    // modyfikacja kąta obrotu o kat proporcjonalny
        fi2 += delta_y * pix2angley;
    }

    //Ustalenie położenia źródła swiatła pierwszego według instrukcji
    light_position[0] = zoom * cos(theta1) * cos(fi);
    light_position[1] = zoom * sin(fi);
    light_position[2] = zoom * sin(theta1) * cos(fi);
    //Ustalenie położenia źródła swiatła drugiego według instrukcji
    light_positionSecond[0] = zoom * cos(theta2) * cos(fi2);
    light_positionSecond[1] = zoom * sin(fi2);
    light_positionSecond[2] = zoom * sin(theta2) * cos(fi2);
...
void MyInit(void)
{
    ...
    GLfloat mat_diffuse[] = { 5.0, 5.0, 5.0, 1 };
...
void info()
{
    ...
    cout << "W celu modyfikacji polozenia zrodel swiatla nalezy uzyc: " << endl;
    cout << "LPM + ruch myszka - zmiana polozenia pierwszego zrodla swiatla" << endl;
    cout << "PPM + ruch myszka - zmiana polozenia drugiego zrodla swiatla" << endl;
}
...
void main(void)
{
    ...
    glutCreateWindow("Oswietlenie - dwa zrodla");
