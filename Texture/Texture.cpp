...
//Na podstawie http://www.zsk.ict.pwr.wroc.pl/zsk/dyd/intinz/gk/lab/cw_6_dz/
//Styczeń 2022r.
    ...
    *ImHeight = tgaHeader.height;
    ...
    case 1:
        ...
        *ImComponents = GL_LUMINANCE8;
        break;
    case 3:
        //Format BGR, odpowiada pamieci map bitowych Windows (DIB)
        *ImFormat = GL_BGR_EXT;
        *ImComponents = GL_RGB8;
        break;
    case 4:
        //Format BGRA, odpowiada pamieci map bitowych Windows (DIB)
...
//Funkcja glTexCoord2f() poprzez swoje argumenty określa, które wierzchołki trójkąta naniesionego na wzorzec tekstury odpowiadają , którym wierzchołkom teksturowanego trójkąta.
...
void eggPoints()
{
    ...
    for (int u = 0; u < n; ++u)
    {
        for (int v = 0; v < n; ++v)
        {
            ...
            points[u][v][1] = y;
            points[u][v][2] = z;
            texturePoints[u][v][0] = fu;
            texturePoints[u][v][1] = fv;           
        }
    }
}
    ...
    for (int u = 0; u < n; ++u)
    {
        for (int v = 0; v < n; ++v)
        {
            ...
            if (u < n / 2)
            {
                ...
                glTexCoord2fv(texturePoints[u][v + 1 - n * vdiff]);
                glVertex3fv(points[u][(v + 1 - n * vdiff)]);

                glNormal3fv(normals[u][v]);
                glTexCoord2fv(texturePoints[u][v]);
                glVertex3fv(points[u][v]);

                glNormal3fv(normals[u + 1 - n * udiff][v]);
                glTexCoord2fv(texturePoints[u + 1 - n * udiff][v]);
                ...
                glTexCoord2fv(texturePoints[u + 1 - n * udiff][v + 1 - n * vdiff]);
...
{
    if (key == '1') side1 = !side1;
    if (key == '2') side2 = !side2;
    if (key == '3') side3 = !side3;
    if (key == '4') side4 = !side4;
    if (key == '5') side5 = !side5;
    if (key == 'e') model = 0;
    if (key == 'p') model = 1;
    // przerysowanie obrazu sceny
    RenderScene();
...
void MyInit(void)
{
    ...
    //level - Poziom szczegółowości. Jest ustawiany na 0 gdy mipmapy nie są stosowane, gdy mipmapy są używane level jest liczbą określająca poziom redukcji obrazu tekstury.
    //components - Ilość używanych składowych koloru, liczba od 1 do 4.
    //width - Szerokosć obrazu tekstury, zawsze jest potegą liczby 2, może być powiększona o szerokość tak zwanej ramki tekstury
    ...
    //piksels -  Tablica o rozmiarze  width x height x components, w ktorej znajdują się dane z obrazem tekstury.
    glTexImage2D(GL_TEXTURE_2D, 0, ImComponents, ImWidth, ImHeight, 0, ImFormat, GL_UNSIGNED_BYTE, pBytes);
    // Zwolnienie pamięci
    free(pBytes);
    // Włączenie mechanizmu teksturowania
    glEnable(GL_TEXTURE_2D);
    //Funkcja glTexEnvi() służy do ustalenia tak zwanego trybu teksturowania, czyli sposobu łączenia koloru piksela obrazu tekstury z kolorem piksela ekranu.
    ...
    //GL_TEXTURE_MIN_FILTER - Opisuje w jaki sposób następuje pominiejszanie tekstury (usuwanie pikseli)
...
void info()
{
    ...
    cout << "LPM + ruch myszy - Obracanie dookola obiektu" << endl;
    cout << "PPM + ruch myszy - Przyblizanie/Oddalanie sie od obiektu" << endl;
    cout << "W celu modyfikacji modelu nalezy uzyc:" << endl;
    cout << "p - Wyswietlenie piramidy" << endl;
    cout << "e - Wyswietlenie jajka" << endl;
    cout << "1,2,3,4,5 - Pokazanie/ukrycie odpowiedniej sciany piramidy" << endl;
}
...
void main(void)
{
    ...
    texturePoints = new point2 * [n];
    for (int i = 0; i < n; ++i)
    {
        points[i] = new point3[n];
        normals[i] = new point3[n];
        texturePoints[i] = new point2[n];
    }
